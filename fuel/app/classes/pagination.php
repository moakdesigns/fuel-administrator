...
class Pagination extends \Fuel\Core\Pagination
{
	...
	public static $template = array(
		...
		'prevdisabled_start' => '<li class="prev disabled"> ',
		'previous_end'   => ' </li>',
		'previous_mark'  => '&laquo; ',
		'pagelinks_start' => '<li>',
		'pagelinks_end' => '</li>',
		'next_start'     => '<li class="next"> ',
		'nextdisabled_start' => '<li class="next disabled"> ',
		'next_end'       => ' </li>',
		'next_mark'      => ' &raquo;',
	...
	public static function page_links()
	{
		...
		for($i = $start; $i <= $end; $i++)
		{
			...
			{
				$url = ($i == 1) ? '' : '/'.$i;
				$pagination .= static::$template['active_start'].\Html::anchor(rtrim(static::$pagination_url, '/').$url, $i).static::$template['active_end'];
			}
			else
			{
				$url = ($i == 1) ? '' : '/'.$i;
				$pagination .= static::$template['pagelinks_start'].\Html::anchor(rtrim(static::$pagination_url, '/').$url, $i).static::$template['pagelinks_end'];
			}
		}
	...
	public static function next_link($value)
	{
		...
		if (static::$current_page == static::$total_pages)
		{
			return static::$template['nextdisabled_start'].\Html::anchor(rtrim('#'),$value.static::$template['next_mark']).static::$template['next_end'];
		}
		else
		{
	...
	public static function prev_link($value)
	{
		...
		{
			return static::$template['prevdisabled_start'].\Html::anchor(rtrim('#'),$value.static::$template['previous_mark']).static::$template['previous_end'];
		}
		else
		{
